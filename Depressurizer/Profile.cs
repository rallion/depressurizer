...
namespace Depressurizer
{
    public class Profile
    {
        ...
        public void ExportSteamData()
        {
            ...
            if (!IgnoreExternal)
            {
                GameData.ExportSteamShortcuts(SteamID64);
            }
        }
        ...
        public static Profile Load(string path)
        {
            ...
            if (profileNode != null)
            {
                if (profileNode.Attributes != null)
                {
                    ...
                    long accId = XmlUtil.GetInt64FromNode(profileNode["steam_id_64"], 0);
                    if (accId == 0)
                    {
                    ...
                    profile.OverwriteOnDownload = XmlUtil.GetBoolFromNode(profileNode["overwrite_names"],
                        profile.OverwriteOnDownload);
                    profile.IgnoreDlc = XmlUtil.GetBoolFromNode(profileNode["ignore_dlc"], profile.IgnoreDlc);
                    profile.IgnoreExternal = XmlUtil.GetBoolFromNode(profileNode["ignore_external"],
                        profile.IgnoreExternal);
