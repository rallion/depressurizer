...
namespace Depressurizer
{
    public partial class FormMain : Form
    {
        ...
        private object lastSelectedCat;

        // Used to reload resources of main form while switching language
        private int originalHeight, originalSplitDistanceMain, originalSplitDistanceSecondary;
        ...
        private bool ManualExportConfig()
        {
        ...
        private void DownloadProfileData(Int64 steamId, bool overwrite, SortedSet<int> ignore, bool ignoreDlc,
            bool ignoreExternal)
        {
            var updateDlg = new CDlgUpdateProfile(gameData, steamId, overwrite, ignore, ignoreDlc, ignoreExternal);
        ...
        private void DownloadProfileDataHelper(CDlgUpdateProfile updateDlg)
        {
            ...
            if (updateDlg.Error != null)
            {
                ...
                MessageBox.Show(string.Format(GlobalStrings.MainForm_ErrorDowloadingProfile, updateDlg.Error.Message),
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        MessageBox.Show(GlobalStrings.MainForm_NoGameDataFound, GlobalStrings.DBEditDlg_Warning,
                    ...
                    {
                        MakeChange(true);
                        AddStatus(currentProfile.IgnoreExternal
                            ? string.Format(GlobalStrings.MainForm_DownloadedGames, updateDlg.Fetched, updateDlg.Added,
                                updateDlg.UseHtml ? "HTML" : "XML")
                            : string.Format(GlobalStrings.MainForm_DownloadedGamesWithExternal, updateDlg.Fetched,
                                updateDlg.Added, updateDlg.Removed, updateDlg.UseHtml ? "HTML" : "XML"));
        ...
        private void VisitSelectedGameStorePage()
        {
            if (lstGames.SelectedIndices.Count > 0)
            {
                int index = lstGames.SelectedIndices[0];
                var g = lstGames.Items[index].Tag as GameInfo;

                if (g != null)
                {
                    Process.Start(string.Format(Resources.UrlSteamStore, g.Id));
                }
            }
        }
        ...
        private void EditProfile()
        {
            if (ProfileLoaded)
            {
                var dlg = new DlgProfile(currentProfile);
            ...
            {
                if (
                    MessageBox.Show(GlobalStrings.MainForm_NoProfileLoaded, GlobalStrings.DBEditDlg_Error,
        ...
        private void LoadProfile(string path, bool checkForChanges = true)
        {
            ...
            {
                MessageBox.Show(e.Message, GlobalStrings.MainForm_ErrorLoadingProfile, MessageBoxButtons.OK,
        ...
        private bool SaveProfile(string path = null)
        {
            ...
            {
                MessageBox.Show(e.Message, GlobalStrings.MainForm_ErrorSavingProfile, MessageBoxButtons.OK,
        ...
        private void UpdateProfileDownload(bool updateUI = true)
        {
            if (currentProfile != null)
            {
                ...
                try
                {
                    DownloadProfileData(currentProfile.SteamID64, currentProfile.OverwriteOnDownload,
                        currentProfile.IgnoreList, currentProfile.IgnoreDlc, currentProfile.IgnoreExternal);
                }
                catch (ApplicationException e)
                {
                    if (updateUI) Cursor = Cursors.Default;
                    MessageBox.Show(e.Message, GlobalStrings.MainForm_ErrorDownloadingGameList, MessageBoxButtons.OK,
                        MessageBoxIcon.Warning);
        ...
        private void UpdateProfileImport(bool updateUI = true)
        {
            if (currentProfile != null)
            {
                if (updateUI) Cursor = Cursors.WaitCursor;
                ...
                {
                    if (updateUI) Cursor = Cursors.Default;
                    MessageBox.Show(e.Message, GlobalStrings.MainForm_ErrorImportingSteamDataList, MessageBoxButtons.OK,
                        MessageBoxIcon.Warning);
        ...
        private void ProfileExport()
        {
            if (currentProfile != null)
            {
                ...
                {
                    MessageBox.Show(e.Message, GlobalStrings.MainForm_ErrorExportingToSteam, MessageBoxButtons.OK,
                        MessageBoxIcon.Error);
                    AddStatus(GlobalStrings.MainForm_ExportFailed);
                }
            }
        ...
        {
            var dlg = new GetStringDlg(string.Empty, GlobalStrings.MainForm_CreateCategory,
            ...
            if (dlg.ShowDialog() == DialogResult.OK && CatUtil.ValidateCategoryName(dlg.Value))
            {
                ...
                MessageBox.Show(String.Format(GlobalStrings.MainForm_CouldNotAddCategory, dlg.Value),
                    GlobalStrings.DBEditDlg_Warning, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            return null;
        }
        ...
        private bool DeleteCategory()
        {
            ...
            {
                var c = lstCategories.SelectedItem as Category;
                if (c != null)
                {
                    DialogResult res = MessageBox.Show(string.Format(GlobalStrings.MainForm_DeleteCategory, c.Name),
                    ...
                    if (res == DialogResult.Yes)
                    {
                        ...
                        MessageBox.Show(string.Format(GlobalStrings.MainForm_CouldNotDeleteCategory, c.Name),
                            GlobalStrings.DBEditDlg_Warning, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    }
                }
            }
        ...
        private bool RenameCategory()
        {
            if (lstCategories.SelectedItems.Count > 0)
            {
                ...
                if (c != null)
                {
                    var dlg = new GetStringDlg(c.Name, string.Format(GlobalStrings.MainForm_RenameCategory, c.Name),
        ...
        private bool RenameCategoryHelper(Category c, string newName)
        {
            ...
            MessageBox.Show(string.Format(GlobalStrings.MainForm_NameIsInUse, newName), GlobalStrings.DBEditDlg_Warning,
                MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return false;
        }
        ...
        private void AddGame()
        {
            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                FullListRefresh();
                //overkill. only need to change one cat and one game at most. could be replaced with a cat refresh then adding the game if in ungrouped mode
                MakeChange(true);
        ...
        private void EditGame()
        {
            if (lstGames.SelectedIndices.Count > 0)
            {
                int index = lstGames.SelectedIndices[0];
                var g = lstGames.Items[index].Tag as GameInfo;
                var dlg = new DlgGame(gameData, g);
                if (dlg.ShowDialog() == DialogResult.OK)
                {
        ...
        private void AddCategoryToSelectedGames(Category cat, bool refreshCatList, bool forceClearOthers)
        {
            if (lstGames.SelectedItems.Count > 0)
            {
                foreach (ListViewItem item in lstGames.SelectedItems)
                {
                    var g = item.Tag as GameInfo;
                    if (g != null)
                    {
        ...
        private void Unload(bool updateUI = true)
        {
            if (!CheckForUnsaved())
            {
                return;
        ...
        private void Autocategorize(bool selectedOnly, AutoCat autoCat)
        {
            ...
            var gamesToUpdate = new List<GameInfo>();

            if (selectedOnly)
            {
                gamesToUpdate.AddRange(from ListViewItem item in lstGames.SelectedItems
                    select item.Tag as GameInfo
                    into g
                    where (g != null) && (g.Id > 0)
                    select g);
            }
            ...
            {
                gamesToUpdate.AddRange(gameData.Games.Values.Where(g => (g != null) && (g.Id > 0)));
            }

            int updated = 0;

            // List of games not found in database, so we can try to scrape data for them
            var notFound = new List<GameInfo>();
            ...
            if (notFound.Count > 0)
            {
                if (MessageBox.Show(string.Format(GlobalStrings.MainForm_GamesNotFoundInGameDB, notFound.Count),
                    GlobalStrings.DBEditDlg_Confirm, MessageBoxButtons.YesNo, MessageBoxIcon.Question,
                    MessageBoxDefaultButton.Button1)
                    == DialogResult.Yes)
                {
                    var jobs = new Queue<int>();
                    foreach (GameInfo g in notFound) jobs.Enqueue(g.Id);

                    var scrapeDlg = new DbScrapeDlg(jobs);
                    ...
                    else
                    {
                        ...
                        updated +=
                            notFound.Select(autoCat.CategorizeGame).Count(res => res == AutoCatResult.Success);
                    }
                }
            }
            autoCat.DeProcess();
        ...
        {
            statusSelection.Text = string.Format(GlobalStrings.MainForm_SelectedDisplayed, lstGames.SelectedItems.Count,
                lstGames.Items.Count);
        }
        ...
        {
            var sb = new StringBuilder("Depressurizer");
            if (ProfileLoaded)
            {
                sb.Append(" - ");
        ...
        private void FillGameList()
        {
            ...
            if (lstCategories.SelectedItems.Count > 0)
            {
                ...
                {
                    var cat = lstCategories.SelectedItem as Category;
                    foreach (GameInfo g in gameData.Games.Values)
                    {
        ...
        private void AddGameToList(GameInfo g)
        {
            ...
            var item =
                ...
                {
                    strId, g.Name, catName, g.ContainsCategory(gameData.FavoriteCategory) ? "X" : String.Empty,
        ...
        private void FillAllCategoryLists()
        {
            ...
            Category[] catList = gameData.Categories.ToArray();
        ...
        private void UpdateGameCheckStates()
        {
            ...
            else
            {
                ...
                {
                    var game = gameItem.Tag as GameInfo;
                    if (game != null)
                    {
        ...
        private void AddGameToMultiCatCheckStates(GameInfo game, bool first)
        {
            foreach (ListViewItem catItem in lstMultiCat.Items)
            {
                ...
                {
                    var cat = catItem.Tag as Category;
                    if (cat != null)
                    {
                        if (first)
                        {
        ...
        private void AddGameToCheckboxStates(GameInfo game, bool first)
        {
            ignoreCheckChanges = true;
            if (first)
            {
                chkFavorite.CheckState = game.ContainsCategory(gameData.FavoriteCategory)
                    ? CheckState.Checked
            ...
            else
            {
                if (chkFavorite.CheckState != CheckState.Indeterminate)
                {
                    ...
                    {
                        if (chkFavorite.CheckState == CheckState.Unchecked)
                            chkFavorite.CheckState = CheckState.Indeterminate;
                    }
                    else
                    {
                        if (chkFavorite.CheckState == CheckState.Checked)
        ...
        private bool UpdateGame(int index)
        {
            ...
            if (ShouldDisplayGame(g))
            {
                ...
                item.SubItems[2].Text = g.GetCatStringExcept(gameData.FavoriteCategory,
        ...
        ///     Try to avoid calling this directly. Look at OnCategoryChange, OnGameChange, OnViewChange, and FullListRefresh.
        /// </summary>
        private void UpdateGameListSelected()
        {
            int i = 0;
            lstGames.BeginUpdate();
        ...
        private void lstCategories_DragDrop(object sender, DragEventArgs e)
        {
            if (e.Data.GetDataPresent(typeof (int[])))
            {
                ...
                var cat = dropItem as Category;
                if (cat != null)
                {
                    Category dropCat = cat;
                ...
                {
                    var s = dropItem as string;
                    if (s != null)
                    {
                        if (s == GlobalStrings.MainForm_Uncategorized)
                        {
        ...
        {
            var selectedGames = new int[lstGames.SelectedItems.Count];
        ...
        private void menuToolsAutocat_Item_Click(object sender, EventArgs e)
        {
            ...
            if (item != null)
            {
                var autoCat = item.Tag as AutoCat;
                if (autoCat != null)
                {
                    ClearStatus();
        ...
        private void changeLanguageControls(Control c, ComponentResourceManager resources, CultureInfo newCulture)
        {
            if (c != null)
            {
                ...
                {
                    var strip = c as MenuStrip;
                    if (strip != null)
                        foreach (ToolStripDropDownItem mItem in strip.Items)
        ...
        private void menu_Tools_Settings_Click(object sender, EventArgs e)
        {
            ...
            if (actualCulture.Name != Thread.CurrentThread.CurrentUICulture.Name)
            {
                ...
                splitContainer.SplitterDistance = originalSplitDistanceMain;
                splitGame.SplitterDistance = originalSplitDistanceSecondary;
        ...
        private void contextGameAddCat_Category_Click(object sender, EventArgs e)
        {
            ...
            {
                ClearStatus();
                var c = menuItem.Tag as Category;
        ...
        {
            var ac = cmbAutoCatType.SelectedItem as AutoCat;
            if (ac != null)
            {
                ClearStatus();
        ...
        private void cmdGameLaunch_Click(object sender, EventArgs e)
        {
            ...
            if (lstGames.SelectedItems.Count > 0)
            {
                var g = lstGames.SelectedItems[0].Tag as GameInfo;
                LaunchGame(g);
            }
            FlushStatus();
        }
        ...
        private bool CheckForUnsaved()
        {
            ...
            DialogResult res = MessageBox.Show(GlobalStrings.MainForm_UnsavedChangesWillBeLost,
                GlobalStrings.MainForm_UnsavedChanges, MessageBoxButtons.YesNoCancel, MessageBoxIcon.Warning);
            ...
            if (ProfileLoaded)
            {
                ...
                {
                    MessageBox.Show(GlobalStrings.MainForm_SavingProfileDataFailed + e.Message,
                        GlobalStrings.MainForm_ErrorSavingFile, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return false;
                }
            }
        ...
        private bool ShouldDisplayGame(GameInfo g)
        {
            ...
            var s = lstCategories.SelectedItem as string;
            if (s != null)
            {
                if (s == GlobalStrings.MainForm_All)
                {
                    return true;
                }
                if (s == GlobalStrings.MainForm_Uncategorized)
        ...
        private void LaunchGame(GameInfo g)
        {
            if (g != null)
            {
                ...
                Process.Start("steam://rungameid/" + gameIdentifier);
            }
        }
    ...
    internal static class CatUtil
    {
        ...
        public static bool ValidateCategoryName(string name)
        {
            ...
            {
                MessageBox.Show(GlobalStrings.MainForm_CategoryNamesNotEmpty, GlobalStrings.DBEditDlg_Warning,
